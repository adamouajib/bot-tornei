...
- `GEMINI_API_KEY` must be present for the private AI channel created by `:start`. The assistant calls the configured AI service asynchronously through `aiohttp`, using bounded HTTP connections, per-user serialization, Discord typing status and short retries for transient/quota responses. To users it identifies only as the Official PCF™ Server Assistant, created by Adam, and replies in the user's language; it never reveals the provider or model name.
