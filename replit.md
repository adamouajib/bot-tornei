...
- `:1v1 @member` starts a Ruby wager duel. When the challenged member accepts, the bot creates a private thread containing both players and moves the duel controls there; they choose any map and match rules themselves, each wager must be at least 100 Ruby, and winning the duel awards Ruby only (no winner role).
