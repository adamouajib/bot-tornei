...
- The assistant builds its server knowledge from all safe text-based project source/configuration files at request time. Environment files, runtime database/session data, archives, internal agent files, credentials and binary assets are excluded.
- Direct messages do not use the AI assistant; normal DM messages instruct members to use `:start`, which opens the private AI channel in the server. Staff applications do not require Supporter status; members should be active in the server and apply through the ticket panel. `:boost` displays booster perks only and does not perform a boost.
