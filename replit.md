...
- `GEMINI_API_KEY` must be present for the private AI channel created by `:start`. The assistant calls the configured AI service asynchronously through `aiohttp`, using bounded HTTP connections, per-user serialization, a persistent Discord typing indicator and short retries for transient/quota responses. To users it identifies only as the Official PCF™ Server Assistant and replies in the user's language; it does not mention the creator unless the user asks or explicitly brings up Adam, and it never reveals the provider or model name.
