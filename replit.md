...
- Classic tournament registration does not require invite history or a Linked role. Big Tournament registration requires only a linked Stumble Guys account in SQLite.
...
- `:setup-getlink` is admin-only and publishes the English `🔗 PERSONAL INVITE LINK GENERATOR` panel. Its persistent `🔗 Create Link` button creates a unique, permanent server invite and replies only to the requesting member.
- Big Tournament setup collects configurable Rubies + Gems prizes for `1st`, `2nd`, `3rd`, `4th - 6th`, and `7th - 9th` place. The `:end` command accepts up to nine final placements and awards the matching bracket prize to each member.
