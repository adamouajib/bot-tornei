...
- `GEMINI_API_KEY` must be present for DM AI. The assistant uses Google's `gemini-1.5-flash` model through `google-generativeai`; requests are serialized per user, show Discord typing status, and retry transient/rate-limit failures up to three times. The assistant identifies exclusively as the Official PCF™ Server Assistant and replies in the user's language.
